class SliddingGame {
    public gameDataSet: Set<number> = new Set();
    public emptyItemClass: string = 'grid-item-empty';
    public positionMap = [
        [0, 0], [1, 0], [2, 0], [3, 0], 
        [0, 1], [1, 1], [2, 1], [3, 1],
        [0, 2], [1, 2], [2, 2], [3, 2],
    ...
    public attachClickEvent(): void {
        document.querySelector('.play-area').addEventListener('click', (e: any) => {
     ...
     */
    public evaluateGameStatus(): void {
        let index = 0;
        let wonGameFlag: boolean = true;
     ...
     */
    public moveImageSlice(x: number, y: number, dataId: number): boolean {
        ...
        if (
            ...
            delete (itemToBeEmpty as HTMLElement).dataset.id;
            itemGettingImage.classList.add('grid-item-' + dataId);
            itemGettingImage.setAttribute('data-id', dataId as unknown as string);
     ...
     * @returns void 
     */
    public logData(data: string, className: string = ''): void {
     ...
     */
    public getCordinates(dataId: number): number[] {
